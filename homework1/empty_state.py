...
class State:
    ...
    def __init__(
        ...
        self.no_classes = sum(1 for day in DAYS_OF_THE_WEEK[:size[1]] \
            for slot in TIME_SLOTS[:size[0]] for classroom in self.yaml_dict[utils.CLASSROOMS] \
                ...
                for classroom in self.yaml_dict[utils.CLASSROOMS]:
                    if self.schedule[day][slot][classroom] is not None:
                        ...
                        if self.yaml_dict[utils.TEACHERS][teacher][utils.BREAK] is not None:
                            # find the previous slot in which the teacher taught
                            previous_slot = copy.deepcopy(slot)
                            while previous_slot != TIME_SLOTS[0]:
                                previous_slot = (previous_slot[0] - 2, previous_slot[1] - 2)
                                if self.schedule[day][previous_slot][classroom] is not None:
                                    if self.schedule[day][previous_slot][classroom][0] == teacher:
                                        break

                            if previous_slot != TIME_SLOTS[0] and \
                                slot[0] - previous_slot[0] > \
                                    2 * self.yaml_dict[utils.TEACHERS][teacher][utils.BREAK]:
                                constraints += 1
    ...
    def h(self):
        ...
        return self.eval() # TODO

    def g(self):
        ''' Returns the cost to reach the current state. '''
        return self.no_classes # TODO

    def get_schedule(self) -> dict[str, dict[str, dict[str, tuple[str, str]]]]:
        ''' Returns the current schedule. '''
        return self.schedule

    def serialize_schedule(self) -> str:
        ''' Returns the serialization of the current schedule. '''
        serialize_schedule = {}
        for day in DAYS_OF_THE_WEEK[:self.size[1]]:
            serialize_schedule[day] = {}
            for slot in TIME_SLOTS[:self.size[0]]:
                serialize_schedule[day][str(slot)] = {}
                for classroom in self.yaml_dict[utils.CLASSROOMS]:
                    if self.schedule[day][slot][classroom] is not None:
                        teacher, subject = self.schedule[day][slot][classroom]
                        serialize_schedule[day][str(slot)][classroom] = (teacher, subject)
                    else:
                        serialize_schedule[day][str(slot)][classroom] = None

        return json.dumps(serialize_schedule)
